"""
Tools for reading code from files in this repo.

These are intentionally constrained to avoid accidentally leaking secrets from
outside the project directory or returning extremely large files.
"""
...
def _repo_root() -> Path:
    # src/tools/code_reader.py -> src/tools -> src -> repo root
...
def _guess_language(path: Path) -> str:
    # A small mapping for nicer rendering; safe to be incomplete.
...
def grab_code(
    ...
    """
    Read code from a file in this repository so you don't have to paste it.

    By default this tool only allows reading files *within this repo* and
    limits output size to keep tool responses manageable.

    Args:
        path: File path to read. Can be relative to repo root or an absolute path.
    ...
    full_path = (root / requested) if not requested.is_absolute() else requested
    ...
    if file_size is not None and file_size > _HARD_MAX_FILE_BYTES_WITHOUT_RANGE and end_line is None:
        return (
            ...
            f"File: {str(full_path.relative_to(root))}\n"
    ...
    if total_lines == 0:
        return f"Error: file appears to be empty: {str(full_path.relative_to(root))}"
